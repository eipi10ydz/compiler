...
//	std::vector <operand> args_val;
	...
	const int get_add_block() { return block_count++; }
	std::tuple<int, int, int> get_count() { return std::make_tuple(block_count, tmp_count, ok_count); }
	void set_count(const std::tuple<int, int, int> &tmp) { std::tie(block_count, tmp_count, ok_count) = tmp; }
